...
//                setArrivalTimes()
            ...
            sharedViewModel.getTrips(
                ...
                viewLifecycleOwner,
                { trips ->
                    Log.d(TAG, "first one: " + trips.toString())
                    for (i in trips) {
                       sharedViewModel.getNewTripStops(routeStopInfoItem.stopId, i.id).observe(
                           viewLifecycleOwner,
                           { tripStop ->

                               Log.d(TAG, "first one: " + tripStop.toString())
...
//                setArrivalTimes()
        ...
        fun setArrivalTimes() {
            ...
            sharedViewModel.getTrips(routeStopInfoItem.routeId, routeStopInfoItem.directionId).observe(
                viewLifecycleOwner,
                { trips ->
